import React, { useState, useEffect, useCallback } from 'react';
import { supabase } from '../services/supabase';
import { Staff, StaffAttendance } from '../types';
import Spinner from '../components/Spinner';

interface ProcessedRecord extends StaffAttendance {
    presentStaff: Staff[];
    absentStaff: Staff[];
}

const StaffAttendanceReport: React.FC = () => {
    const [allStaff, setAllStaff] = useState<Staff[]>([]);
    const [allProcessedRecords, setAllProcessedRecords] = useState<ProcessedRecord[]>([]);
    const [filteredRecords, setFilteredRecords] = useState<ProcessedRecord[]>([]);
    const [selectedRecord, setSelectedRecord] = useState<ProcessedRecord | null>(null);
    const [filters, setFilters] = useState({ date: '', month: '' });
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);

    const fetchDataAndProcess = useCallback(async () => {
        setLoading(true);
        setError(null);
        try {
            const [attendanceRes, staffRes] = await Promise.all([
                supabase.from('staff_attendence').select('*').order('date', { ascending: false }),
                supabase.from('staff').select('*').eq('is_active', true)
            ]);

            if (attendanceRes.error) throw attendanceRes.error;
            if (staffRes.error) throw staffRes.error;

            const attendanceData: StaffAttendance[] = attendanceRes.data;
            const staffData: Staff[] = staffRes.data;
            setAllStaff(staffData);

            const staffMap = new Map(staffData.map(s => [s.staff_id, s]));

            const processed = attendanceData.map(record => {
                const presentIds = new Set(record.staff_id ? record.staff_id.split(',') : []);
                const presentStaff = Array.from(presentIds).map(id => staffMap.get(id)).filter((s): s is Staff => !!s);
                const absentStaff = staffData.filter(s => !presentIds.has(s.staff_id));
                return { ...record, presentStaff, absentStaff };
            });

            setAllProcessedRecords(processed);
            setFilteredRecords(processed);
        } catch (err: any) {
            setError(err.message);
        } finally {
            setLoading(false);
        }
    }, []);

    useEffect(() => {
        fetchDataAndProcess();
    }, [fetchDataAndProcess]);

    useEffect(() => {
        let records = [...allProcessedRecords];
        if (filters.date) {
            records = records.filter(r => r.date === filters.date);
        }
        if (filters.month) {
            records = records.filter(r => r.date.startsWith(filters.month));
        }
        setFilteredRecords(records);
    }, [filters, allProcessedRecords]);

    const handleFilterChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        const { name, value } = e.target;
        setFilters(prev => {
            const newFilters = { ...prev, [name]: value };
            if (name === 'date' && value) newFilters.month = '';
            if (name === 'month' && value) newFilters.date = '';
            return newFilters;
        });
    };
    
    const clearFilters = () => setFilters({ date: '', month: '' });

    if (loading) {
        return <div className="flex justify-center items-center h-96"><Spinner size="12" /></div>;
    }

    if (error) {
        return <div className="text-center text-red-500 bg-red-100 p-4 rounded-lg">{error}</div>;
    }
    
    if (selectedRecord) {
        return (
            <div className="bg-white p-8 rounded-xl shadow-lg">
                <button onClick={() => setSelectedRecord(null)} className="mb-4 px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors">
                    &larr; Back to Report List
                </button>
                <div className="border-b pb-4 mb-4">
                    <h2 className="text-2xl font-bold text-gray-800">Staff Attendance Details</h2>
                    <p className="text-gray-600">
                        <span className="font-semibold">Date:</span> {new Date(selectedRecord.date).toLocaleDateString()}
                    </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <h3 className="text-xl font-semibold text-green-600 mb-3">Present ({selectedRecord.presentStaff.length})</h3>
                        <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                           {selectedRecord.presentStaff.length > 0 ? selectedRecord.presentStaff.map(s => (
                               <div key={s.id} className="flex items-center gap-3 bg-gray-50 p-2 rounded-md">
                                   <img src={s.photo_url || `https://ui-avatars.com/api/?name=${s.name}`} alt={s.name} className="w-12 h-12 rounded-full object-cover"/>
                                   <div>
                                       <p className="font-semibold">{s.name}</p>
                                       <p className="text-sm text-gray-500">{s.staff_id}</p>
                                   </div>
                               </div>
                           )) : <p className="text-gray-500">No staff were marked present.</p>}
                        </div>
                    </div>
                     <div>
                        <h3 className="text-xl font-semibold text-red-600 mb-3">Absent ({selectedRecord.absentStaff.length})</h3>
                         <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                           {selectedRecord.absentStaff.length > 0 ? selectedRecord.absentStaff.map(s => (
                               <div key={s.id} className="flex items-center gap-3 bg-red-50 p-2 rounded-md">
                                   <img src={s.photo_url || `https://ui-avatars.com/api/?name=${s.name}`} alt={s.name} className="w-12 h-12 rounded-full object-cover"/>
                                   <div>
                                       <p className="font-semibold">{s.name}</p>
                                       <p className="text-sm text-gray-500">{s.staff_id}</p>
                                   </div>
                               </div>
                           )) : <p className="text-gray-500">All staff were present.</p>}
                        </div>
                    </div>
                </div>
            </div>
        )
    }

    return (
        <div className="bg-white p-8 rounded-xl shadow-lg">
            <h1 className="text-3xl font-bold text-gray-800 mb-6">Staff Attendance Report</h1>
            
            <div className="flex flex-wrap gap-4 items-center p-4 bg-gray-50 rounded-lg mb-6">
                <div className="flex-1 min-w-[150px]">
                    <label htmlFor="date" className="block text-sm font-medium text-gray-700">Specific Date</label>
                    <input type="date" id="date" name="date" value={filters.date} onChange={handleFilterChange} className="mt-1 input-field" />
                </div>
                <div className="flex-1 min-w-[150px]">
                    <label htmlFor="month" className="block text-sm font-medium text-gray-700">Month</label>
                    <input type="month" id="month" name="month" value={filters.month} onChange={handleFilterChange} className="mt-1 input-field" />
                </div>
                <div className="pt-6">
                    <button onClick={clearFilters} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-100">Clear Filters</button>
                </div>
            </div>

            <div className="overflow-x-auto">
                <table className="min-w-full bg-white border border-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="th">Date</th>
                            <th className="th">Present</th>
                            <th className="th">Absent</th>
                            <th className="th text-center">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                        {filteredRecords.length > 0 ? filteredRecords.map(record => (
                            <tr key={record.id}>
                                <td className="td font-semibold">{new Date(record.date).toLocaleDateString()}</td>
                                <td className="td text-green-600">{record.presentStaff.length}</td>
                                <td className="td text-red-600">{record.absentStaff.length}</td>
                                <td className="td text-center">
                                    <button onClick={() => setSelectedRecord(record)} className="px-3 py-1 bg-primary text-white text-sm rounded-md hover:bg-primary-dark">
                                        View Details
                                    </button>
                                </td>
                            </tr>
                        )) : (
                            <tr><td colSpan={4} className="text-center py-8 text-gray-500">No records match the current filters.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            <style>{`
                .input-field { display: block; width: 100%; padding: 0.5rem 0.75rem; border-radius: 0.375rem; border: 1px solid #D1D5DB; }
                .th { padding: 0.75rem 1rem; text-align: left; font-size: 0.75rem; font-weight: 500; color: #6B7280; text-transform: uppercase; }
                .td { padding: 1rem 1rem; font-size: 0.875rem; white-space: nowrap; }
            `}</style>
        </div>
    );
};

export default StaffAttendanceReport;
